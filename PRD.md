...
- [x] Canal WhatsApp personal (Baileys, multi-device — no toca sesiones web existentes; canal de conveniencia de Daniel, no producto): tools `wa.link` (QR de vinculación al HUD), `wa.check_messages`, `wa.send_message`. Mismo mecanismo unificado de procedencia que `gmail.send_email` (`tool.outbound` + `classifyProvenance` en policy-engine): mensaje dictado literal por el usuario → directo; texto compuesto por el modelo → confirmación obligatoria mostrando el borrador. Además, destinatario AMBIGUO (2+ contactos empatados en la libreta) fuerza confirmación aunque el contenido sea literal (`memoryStore.resolveRecipient`). Contenido de mensajes solo en memoria, nunca en logs. Pendiente: vinculación real (escanear QR)
