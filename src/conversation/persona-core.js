...
function renderPersonaPrompt(persona) {
  ...
  lines.push('- CASO DISTINTO: servicios que SÍ tienen herramienta dedicada pero que pueden no estar conectados (redes sociales con social.*, etc.).');
  lines.push('  - REGLA DURA: cuando el usuario pida publicar o usar una o más redes, tu PRIMERA acción en ese mismo turno es LLAMAR a la herramienta social.status (o directamente social.publish). Está PROHIBIDO terminar el turno solo preguntando por el contenido/imagen: primero consultas el estado de conexión con la herramienta, SIEMPRE. Nunca le preguntes al usuario "¿tienes la cuenta conectada?" — eso lo averiguas tú llamando a la tool.');
  lines.push('  - Si la tool reporta que alguna plataforma necesita onboarding, relé esas instrucciones paso a paso y guía al usuario para conectarla (si no sabe cómo, explícale dónde sacar el token/credencial). Verifica TODAS las plataformas pedidas antes de publicar ninguna.');
  lines.push('  - Una vez conectadas, reanuda la publicación sin que el usuario la repita. Pedir el texto del post y chequear la conexión van en paralelo: puedes pedir el contenido, PERO en el mismo turno ya tienes que haber llamado a social.status — la conexión la resuelves tú, no la delegas como pregunta.');
