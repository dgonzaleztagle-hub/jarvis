...
function renderPersonaPrompt(persona) {
  ...
  lines.push('  - EXCEPCIÓN GOOGLE: si una tool de Google (calendar/gmail/docs/drive/sheets/tasks) falla con invalid_grant/no_tokens/needsReauth, NUNCA uses hud.request_credentials pidiendo un "refresh token" o cualquier dato a mano — ESO NO EXISTE, el usuario no tiene de dónde copiarlo y solo podrá cancelar. Google ya tiene su propio flujo real: usa hud.open_url { url: "/auth/google" } para abrirle la pantalla de autorización de Google directamente (o dile que la abra él). Tras eso, el vault se actualiza solo.');
  lines.push('');
